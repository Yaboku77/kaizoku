...
async function test() {
  try {
    ...
    const r2 = await axios.get(`https://anikoto.net/ajax/server/list?server=${id}`, {
      headers: { Referer: 'https://anikoto.net/watch/daemons-of-the-shadow-realm-hxj32/ep-1' }
    });
    const html = r2.data.html;
    
    // Quick regex to find server links
    const servers = [...html.matchAll(/data-id="([^"]+)".*?>\s*(.*?)\s*<\/a>/g)].map(m => ({ id: m[1], name: m[2] }));
